...
func TestMCPCommandsUseUserIdentityAndExpectedEndpoints(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			responseBody: `{"code":0,"data":[{"contract_number":"CN-001"}]}`,
			wantContains: []string{`"contract_number": "CN-001"`},
		},
		{
			name:         "contract sync-user-groups",
			args:         []string{"contract", "sync-user-groups", "--profile", "contract-group"},
			wantMethod:   http.MethodPost,
		...
		{
			name:         "contract text",
			args:         []string{"contract", "text", "contract-1", "--profile", "contract-group", "--full-text"},
			wantMethod:   http.MethodGet,
		...
		},
		{
			...
			args:         []string{"contract", "create", "--profile", "contract-group", "--data", `{"title":"demo"}`},
			wantMethod:   http.MethodPost,
		...
		},
		{
			...
			args:         []string{"contract", "template", "list", "--profile", "contract-group", "--category-number", "CAT-1", "--page-size", "20"},
			wantMethod:   http.MethodGet,
		...
		},
		{
			...
			args:         []string{"contract", "template", "get", "tpl-1", "--profile", "contract-group"},
			wantMethod:   http.MethodGet,
		...
		},
		{
			...
			args:         []string{"contract", "template", "instantiate", "--profile", "contract-group", "--input-file", templateFile},
			wantMethod:   http.MethodPost,
		...
		},
		{
			...
			args:         []string{"mdm", "vendor", "list", "--profile", "contract-group", "--name", "供应商", "--page-size", "10"},
			wantMethod:   http.MethodGet,
		...
		{
			name:         "mdm vendor get",
			args:         []string{"mdm", "vendor", "get", "vendor-1", "--profile", "contract-group"},
			wantMethod:   http.MethodGet,
		...
		{
			name:         "mdm legal list",
			args:         []string{"mdm", "legal", "list", "--profile", "contract-group", "--name", "主体A", "--page-size", "10"},
			wantMethod:   http.MethodGet,
		...
		{
			name:         "mdm legal get",
			args:         []string{"mdm", "legal", "get", "entity-1", "--profile", "contract-group"},
			wantMethod:   http.MethodGet,
		...
		},
		{
			...
			args:         []string{"mdm", "fields", "list", "--profile", "contract-group", "--biz-line", "vendor"},
			wantMethod:   http.MethodGet,
...
func TestStructuredMCPCommandRejectsBotIdentity(t *testing.T) {
	t.Parallel()
	...
	err := app.Run(context.Background(), []string{
		"contract", "get", "contract-1", "--profile", "contract-group", "--as", "bot",
	})

...
2026-09-22 根据业务反馈补齐 Agent 导入前置语义：业务 Excel 只作为行数据来源，不能替代用途和矩阵结构确认；用途不明确时先询问，不执行创建、改列或导入计划。明确用于协商自动邀请时，Agent 必须先确保存在 `value_type=COLLECTION` 的 `合同类型` 条件列，结构变更单独确认并重读列头后再导入。CLI 仍只接收 import JSON，不新增原生 Excel 文件接口。
...
列容量与服务端保持一致：总列数上限 12，优先级和备注固定占 2 列，条件列与结果列合计最多 10 列。Agent 在首个结构写入前按完整目标方案计算；原子 `column add` 在 POST 前通过列头接口统计当前容量，满额时本地阻断并保留服务端 20303 作为并发兜底。

当前部门契约已收敛：人员使用正 int64 外部 ID；部门目录保留数字 `department_id` 作为 batch-get 查询键，同时返回规则行可直接使用的 `open_department_id`（`od-...`）；行操作和批量导入只接受后者。NUMBER 使用最多 30 位整数、8 位小数的精确十进制；新增列通过基准列与方向插入，再更新配置；已有值的类型变更先由服务端阻止。

导入计划升级 v3：列头和全量规则行摘要、应用/环境指纹、24 小时有效期；plan 分页读取规则行，按当前行加 create 数预检 2000 行上限并验证 update 目标行；apply 前重读列头和行快照，请求前记录 running，写成功后先记 unverified 并回读，核验吻合才记 success。回读失败由 verify 只读重试；pause 在当前行完成后停批，resume 清除暂停标记。uncertain 立即停批；HTTP 401/403/429 停批，404/409/412 失效，服务端 20302 超行数使计划失效。保留 --rows、--batch-size、get/cancel 和成功行跳过。

尚未实现服务端原子 revision 条件写、跨计划去重、uncertain 自动核验闭环及发布确认令牌；这些交互由 Agent 查询、确认和分批调用承接。后端详情当前仅含基础列头，缺少操作符/关联元素/默认值的完整回读，不能把列头摘要等同于完整配置快照。未完成的旧版计划需重建，已完成计划可读取原结果。

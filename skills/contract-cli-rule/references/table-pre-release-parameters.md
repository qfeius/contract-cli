...
- profile 重建等原因导致本地基准丢失，而服务端详情为 `status=0` 且存在 `prepared_version` 时，本命令进入只读恢复：前后两次核对版本、状态和完整规则行，稳定后保存本地基准并返回 `baseline_recovered=true`。该分支不调用服务端 `pre_release`，也不写任何规则行。
- 收到 `baseline_recovered=true` 后，Agent 应重新展示 `prepared_version` 与 `row_count` 并取得正式发布确认。不得通过原值重写、清空或增删规则行重置矩阵状态。
